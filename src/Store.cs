...
static class Store
{
    ...
    internal static Product GetProduct(string productId)
    {
    ...
    internal static IList<UpdateIdentity> GetUpdates(Product product, bool _ = false)
    {
        ...
        foreach (XmlNode node in result.GetElementsByTagName("SecuredFragment"))
        {
            ...
            if (_ || package is null || (!package.IsDevelopmentMode &&
                new Version(item.Version) > new Version(package.Id.Version.Major, package.Id.Version.Minor, package.Id.Version.Build, package.Id.Version.Revision)))
            {
                ...
                updates.Add(new()
                {
                    Path = Path.Combine(path, $"{item.PackageFamilyName}.zip"),
